...
use crate::domain::events::{TradeSignalEvent, OrderStatusEvent};
...
impl ExecutionManager {
 ...
 pub async fn start(&self, callback: fn(OrderStatusEvent)) -> Result<(), Error> {
     ...
     let wrapped_callback = move |status: OrderStatusEvent| {
         if let Some(order_id) = status.order_id.clone() {
 ...
 fn test_validate_trade_signal() {
     ...
     assert!(ExecutionManager::validate_trade_signal(&invalid_side).is_err());
 }
 }
}
EOF"
