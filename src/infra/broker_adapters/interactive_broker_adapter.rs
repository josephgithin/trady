...
impl InteractiveBrokerAdapter {
    ...
    fn validate_order(order: &Order) -> Result<(), Error> {
        ...
        if order.order_type == "LIMIT" && order.price <= 0.0 {
 ...
 async fn place_order(&self, order: Self::OrderType) -> Result<(), Error> {
     ...
     tokio::spawn(async move {
         ...
         let filled_event = OrderStatusEvent {
             ...
             remaining_size: 0.0,
             fill_price: if order.order_type == "LIMIT" {
                 order.price
             } else {
                 9999.99 // Mock market price
             },
             reason: String::new(),
         };
 ...
 mod tests {
     ...
     fn test_adapter_creation() {
         let config = BrokerConfig {
             broker_api: "https://api.interactivebrokers.com".to_string(),
         };
